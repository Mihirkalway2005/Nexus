...
export default function SignupPage() {
  ...
  const onSubmit = async (data: SignupValues) => {
    ...
    try {
      ...
      if (response.error) {
        setErrorMsg(
          response.error.message || "An account with this email already exists."
        );
      } else {
    ...
    try {
      ...
      if (res?.error) {
        if (res.error.code === "PROVIDER_NOT_FOUND" || res.error.message?.includes("Provider not found")) {
          setErrorMsg(
            "Google Sign-In is not configured yet. Please set your Google Client ID & Secret in Convex, or create an account with email and password."
          );
        } else {
        ...
        <Card className="rounded-md border-zinc-800 bg-[#0c0c0e] shadow-2xl">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-xs font-mono text-zinc-500">
              Create student identity on the unified system
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div className="space-y-1.5">
                <Label htmlFor="name" className="font-mono text-xs text-zinc-300">
                  Full Name
                </Label>
                <Input
                  id="name"
                  type="text"
                  placeholder="Mihir Kalway"
                  {...register("name")}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="confirmPassword" className="font-mono text-xs text-zinc-300">
                    Confirm
                  </Label>
                  <Input
                    id="confirmPassword"
